...
func TestPutGetRoundtripKeychain(t *testing.T) {
	...
	got, err := Put("alpha", BackendKeychain, "mt_TEST_alpha_long")
	if err != nil {
		t.Fatalf("Put: %v", err)
	}
...
func TestGetFileBackendUsesFileSecret(t *testing.T) {
	...
	v, err := Get("alpha", BackendFile, "mt_TEST_file_secret")
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	if v != "mt_TEST_file_secret" {
...
func TestGetLegacyEmptyBackendUsesFileSecret(t *testing.T) {
	...
	v, err := Get("legacy", "", "mt_TEST_legacy_in_file")
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	if v != "mt_TEST_legacy_in_file" {
...
func TestDeleteIsIdempotent(t *testing.T) {
	...
	if _, err := Put("alpha", BackendKeychain, "mt_TEST_alpha_long"); err != nil {
		t.Fatal(err)
	}
...
func TestKeychainAvailableHonorsDisableEnv(t *testing.T) {
	...
	if KeychainAvailable() {
		t.Error("MOMENTUMTICKER_DISABLE_KEYCHAIN=1 should force unavailable")
	}
}
...
func TestResolveBackendEnvVarOverridesEmpty(t *testing.T) {
	...
	if got != BackendFile {
		t.Errorf("MOMENTUMTICKER_STORAGE=file with empty arg = %q, want file", got)
	}
}
