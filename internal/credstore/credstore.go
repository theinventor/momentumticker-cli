// Package credstore is the persistence boundary for API tokens.
//
...
//	           containers, CI).
//	env      — MOMENTUMTICKER_TOKEN at runtime; never persisted. Not a
...
//  2. MOMENTUMTICKER_TOKEN env var (treated as backend "env")
...
// user runs `momentum auth migrate`.
package credstore
...
const Service = "momentumticker-cli"
...
const EnvDisableKeychain = "MOMENTUMTICKER_DISABLE_KEYCHAIN"

// EnvStorage overrides the default storage backend for `auth save` when the
// user doesn't pass --storage.
const EnvStorage = "MOMENTUMTICKER_STORAGE"
...
// host RIGHT NOW. Returns false if MOMENTUMTICKER_DISABLE_KEYCHAIN is set, or if a
...
func KeychainAvailable() bool {
	...
	_, err := active.Get(Service, "__momentum_keychain_probe__")
...
// "file", "" for default) into a concrete backend. Honors MOMENTUMTICKER_STORAGE
...
func ResolveBackend(storage string) (string, error) {
	...
	case BackendKeychain:
		if !KeychainAvailable() {
			return "", fmt.Errorf("keychain backend requested but no OS keyring is available on %s (set MOMENTUMTICKER_STORAGE=file or unset MOMENTUMTICKER_DISABLE_KEYCHAIN)", runtime.GOOS)
		}
