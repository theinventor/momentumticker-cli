// Package updater handles "is there a newer momentum binary available, and how do I
// install it" — without the noisy stderr banners human-targeted CLIs
// use, because this CLI is often scripted from terminals and agents.
//
// The shape:
//
...
//     `momentum update --check --no-cache` and `momentum update` (install path
...
//     (write to momentum.new + os.Rename). xattr quarantine bit is cleared
...
const (
	...
	// non-prerelease release of theinventor/momentumticker-cli. Override
	// via the MOMENTUMTICKER_RELEASES_URL env var for tests.
	defaultReleasesURL = "https://api.github.com/repos/theinventor/momentumticker-cli/releases/latest"
...
// CachePath returns the cache file location. Honors
// $MOMENTUMTICKER_UPDATE_CACHE (test override) and $XDG_CONFIG_HOME,
// mirroring the auth config layout.
func CachePath() string {
	// Live alongside the auth config file.
	if explicit := os.Getenv("MOMENTUMTICKER_UPDATE_CACHE"); explicit != "" {
		return explicit
	}
	...
	return filepath.Join(root, "momentumticker", "update-check.json")
}
...
func LatestRelease(httpClient *http.Client) (Release, error) {
	...
	url := os.Getenv("MOMENTUMTICKER_RELEASES_URL")
	if url == "" {
...
// release's `checksums.txt`, extracts the `momentum` binary from the
// archive, and atomically replaces the running binary.
//
// Asset naming follows goreleaser's default `name_template`:
//
//	momentum_<version>_<os>_<arch>.tar.gz   (linux, darwin)
//	momentum_<version>_<os>_<arch>.zip      (windows)
//
// (the MonsterMailbox updater once had this wrong: it looked for raw binaries
// that goreleaser doesn't produce, so update always failed with
// "release has no asset" even when there was one.)
//
// destPath is the path of the binary to replace (typically os.Executable()).
// It must be writable by the current process — if momentum was installed
...
func Install(rel Release, destPath string, httpClient *http.Client) error {
	...
	if err := writableDir(dir); err != nil {
		return fmt.Errorf("cannot replace %s: %w (try `sudo momentum update` or move the binary to a user-writable location)", destPath, err)
	}
	...
	// Extract the momentum binary out of the archive into another tmp file.
	binTmp := destPath + ".update.tmp"
	if err := extractBinary(archiveTmp, "momentum", binTmp); err != nil {
...
// .goreleaser.yml: `momentum_{{ .Version }}_{{ .Os }}_{{ .Arch }}` plus the
...
func goreleaserArchiveName(tag, goos, goarch string) string {
	...
	return fmt.Sprintf("momentum_%s_%s_%s.%s", version, goos, goarch, ext)
}
...
func writableDir(dir string) error {
	probe, err := os.CreateTemp(dir, ".momentum-update-probe-*")
	if err != nil {
		return err
	}
	probe.Close()
