...
func TestCheckForUpdate_HitsAPIAndCaches(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(Release{
			...
			Assets:  []Asset{{Name: "momentum-darwin-arm64", BrowserDownloadURL: "https://example/bin"}},
		})
	}))
	defer srv.Close()
	t.Setenv("MOMENTUMTICKER_RELEASES_URL", srv.URL)
...
// (or .zip on Windows) containing `momentum` (or `momentum.exe`) with the given
...
func TestGoreleaserArchiveNamePinsTheNamingConvention(t *testing.T) {
	// The updater must look for Goreleaser archives, not raw binaries.
	// Pin the
	// pattern so a future regression can't slip through review.
	cases := []struct {
		tag, goos, goarch, want string
	}{
		{"v0.2.0", "darwin", "arm64", "momentum_0.2.0_darwin_arm64.tar.gz"},
		{"v0.2.0", "darwin", "amd64", "momentum_0.2.0_darwin_amd64.tar.gz"},
		{"v0.2.0", "linux", "amd64", "momentum_0.2.0_linux_amd64.tar.gz"},
		{"v0.2.0", "linux", "arm64", "momentum_0.2.0_linux_arm64.tar.gz"},
		{"v0.2.0", "windows", "amd64", "momentum_0.2.0_windows_amd64.zip"},
		{"v1.10.3", "darwin", "arm64", "momentum_1.10.3_darwin_arm64.tar.gz"},
		// Both v-prefixed and bare versions should produce the same
		// asset name (goreleaser strips the v).
		{"0.2.0", "linux", "amd64", "momentum_0.2.0_linux_amd64.tar.gz"},
	}
	for _, c := range cases {
...
func TestInstall_VerifiesChecksumAndReplaces(t *testing.T) {
	dir := t.TempDir()
	dest := filepath.Join(dir, "momentum")
...
func TestInstall_FailsWhenArchiveMissingForPlatform(t *testing.T) {
	...
	rel := Release{
		...
		Assets: []Asset{
			...
			{Name: "momentum-" + runtime.GOOS + "-" + runtime.GOARCH, BrowserDownloadURL: srv.URL + "/x"},
...
func TestInstall_FailsWhenArchiveDoesNotContainBinary(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		zw := zip.NewWriter(&buf)
		fw, _ := zw.Create("not-momentum.exe")
		_, _ = fw.Write([]byte("x"))
		_ = zw.Close()
	} else {
		gz := gzip.NewWriter(&buf)
		tw := tar.NewWriter(gz)
		_ = tw.WriteHeader(&tar.Header{Name: "not-momentum", Mode: 0o755, Size: 1, Typeflag: tar.TypeReg})
		_, _ = tw.Write([]byte("x"))
		_ = tw.Close()
		_ = gz.Close()
	}
	...
	if err == nil || !strings.Contains(err.Error(), "does not contain") {
		t.Errorf("expected extract error when archive lacks momentum binary, got %v", err)
	}
	got, _ := os.ReadFile(dest)
	if string(got) != "ORIGINAL" {
...
func TestCacheTTL_StaleCacheIsRefreshed(t *testing.T) {
	...
	t.Setenv("MOMENTUMTICKER_UPDATE_CACHE", cachePath)
